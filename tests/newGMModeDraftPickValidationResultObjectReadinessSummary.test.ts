...
const UNTOUCHED_VALIDATION_RESULT_OBJECT_READINESS_DATABASE =
  "data/saves/__new-gm-mode-draft-pick-validation-result-object-readiness-should-not-exist.sqlite";
const explicitInput = {
  ...
  draftRound: 1,
  draftPickNumber: 3,
  validationStatus: "validation-result-blocked-real-validation-unavailable" as const,
  issueIds: ["selection-intent-invalid", "validation-not-implemented"] as const
};
const validationResultObject =
  createNewGMModeDraftPickValidationResultObject(explicitInput);
const summary = createNewGMModeDraftPickValidationResultObjectReadinessSummary({
  validationResultObject
});

describe("New GM Mode Draft Pick Validation Result Object Readiness Summary v0.1", () => {
  it("reports valid validation result object as ready while pick creation remains unavailable", () => {
    assert.equal(
      summary.draftPickValidationResultObjectReadinessSummaryId,
    ...
    assert.equal(summary.validationResultObjectAvailable, true);
    assert.equal(
  ...
  it("reports invalid phase for malformed injected validation result objects", () => {
    const malformed = JSON.parse(JSON.stringify(validationResultObject));
  ...
  it("preserves injected validationStatus and issueIds without evaluating them", () => {
    assert.equal(
      summary.preservedValidationStatus,
      "validation-result-blocked-real-validation-unavailable"
    );
    assert.deepEqual(summary.preservedIssueIds, [
  ...
  it("keeps draft pick creation, execution, assignment, persistence, mutation, gameplay start, and Week 1 unlock blocked", () => {
    assert.equal(summary.capabilityFlags.validationResultObjectAvailable, true);
    assert.equal(summary.capabilityFlags.canCreateDraftPick, false);
  ...
  it("does not expose selected wrestler, selected candidate object, draft pick IDs or objects, state, persistence, UI, generated text, GenAI, or action payload fields", () => {
    ...
    assert.equal(
      existsSync(UNTOUCHED_VALIDATION_RESULT_OBJECT_READINESS_DATABASE),
      false
    );
  });

  it("exports the validation result object readiness summary from the domain barrel", () => {
    assert.equal(
      typeof createNewGMModeDraftPickValidationResultObjectReadinessSummary,
      "function"
    );
  });
  ...
  it("keeps existing engine behavior, engine IDs, and metadata unchanged", () => {
    ...
    const contextSeed =
      "new-gm-mode-draft-pick-validation-result-object-readiness-no-engine-change";
